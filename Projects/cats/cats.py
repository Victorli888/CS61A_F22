...
def pick(paragraphs, select, k):
    ...
    Select is a function returns true for items in paragraph that can be selected
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # Create a new List, and filter out items that fail our selection function
    word_list = list(filter(select, paragraphs))
    if k < len(word_list):
        return word_list[k]
    return ''
...
def accuracy(typed, source):
    ...
    """
    Cases:
    * Zero Typed Words & Zero Source Words Returns 100%
    * Zero Typed Words & But Some amount of Source Words 0%
    * Some Typed Words & Zero Source Words 0%
    * Some Typed Words & Some Source Words Calculate%
    """
    # BEGIN PROBLEM 3
    if len(typed_words) == 0:
        if len(source_words) == 0:
            return 100.0
        return 0.0
    return sum(map(eq, typed_words, source_words)) / len(typed_words) * 100
