...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if rollValue == 1:
            rolledOne = True
        scoreTotal += rollValue
    if rolledOne:
        scoreTotal = 1
        return scoreTotal
    return scoreTotal

    # Sad Sow: If a player rolls a 1 for any score their total score is 1.
    # here we calculate score of a roll, and if 1 occurs we set rolledOne to true
...
def more_boar(player_score, opponent_score):
    ...
    if len(str_player_score) < 2:
        str_player_score = "0" + str_player_score
    if len(str_opponent_score) < 2:
        str_opponent_score = "0" + str_opponent_score

    p0, p1 = int(str_player_score[0]), int(str_player_score[1])
    o0, o1 = int(str_opponent_score[0]), int(str_opponent_score[1])
    if p0 < o0 and p1 < o1:
        return True
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            numDice = strategy0(score0, score1)
            score0 += take_turn(numDice, score1, dice, goal)
            say = say(score0, score1)
            if more_boar(score0, score1):
                continue
        elif who == 1:
            numDice = strategy1(score1, score0)
            score1 += take_turn(numDice, score0, dice, goal)
            say = say(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        assert who != 1 or who != 0, "who value should only either be 1 or 0"

        running = running_high
        if score > last_score:
            running = score - last_score
            if running > running_high:
                print(f'Player {who} has reached a new maximum point gain. {running} point(s)!')
    ...
    #             running = score0 - last_score
    #             print(f'Player {who} has reached a new maximum point gain. {running} point(s)!')
    #         lastscore = score0
    #     if who == 1 :
    #         if score1 - last_score > running_high :
    #             running = score1 - last_score
    #             print(f'Player {who} has reached a new maximum point gain. {running} point(s)!')
    #         lastscore = score1
    #     return announce_highest(who, lastscore, running)
    # return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_score = mean(1, dice)
    best_rolls = 1
    for num_rolls in range(2, 11):
        score = mean(num_rolls, dice)
        if score > best_score:
            best_score = score
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if more_boar(score+piggy_points(score), opponent_score) is True and piggy_points(opponent_score) >= cutoff:
        return 0
    if piggy_points(opponent_score) >= cutoff or more_boar(score + piggy_points(opponent_score), opponent_score) is True:
        return 0
    return num_rolls
    # END PROBLEM 11
