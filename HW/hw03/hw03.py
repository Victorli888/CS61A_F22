...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def ball(result, i, step):
        """
        PingPong ball starting at position 1 and that changes in the positive or negative
        direction if the number is a factor of 8 or has 8 in the number

        :param result: The result of our ball at nth element in the pingPong sequence
        :param i: our total iterations made
        :param step: move by 1 either in the positve or negative direction
        :return:
        """
        if i == n:
            return result
        elif i % 8 == 0 or num_eights(i) > 0:
            return ball(result - step, i + 1, -step)
        else:
            return ball(result + step, i + 1, step)
    return ball(1, 1, 1)
...
def count_coins(change):
    ...
    def constrained_count(change, smallest_coin):
        ...
        if smallest_coin == None:
            return 0
        # Count the number of ways to count the change without smaller coin
        without_coin = constrained_count(change, next_larger_coin(smallest_coin))
        # Count the number of ways to count the change with the smallest coin
